...
## 1.1 产品边界(诚实声明,比能力清单更重要)

- **是 Copilot,不是自主决策 agent**:定位是风控分析员的加速器。LLM 不进
  在线判定链路(ms 级 SLA、确定性、可对账,三条它都不满足)——在线判定
  永远由引擎(唯一事实源)完成,agent 只读 + 提案 + 人审批。
- **是 reference skeleton,不是生产平台**:缺 Kafka/CDC/实时特征平台/
  特征一致性/模型 serving/引擎 HA/Redis/OLAP/血缘/IAM/KMS/审计不可
  篡改/限流/多租户隔离/SLA/容灾/回放 —— 接真实系统时由基础设施层承担
  (路线见 DEPLOY.md 四步走)。
- **规则是确定性基线,不是真实策略全集**:R001-R006 为 eval 的可复现
  基线而设计;真实风控 = 规则 + 模型 + 图谱 + 设备情报 + 行为序列 +
  画像 + 场景策略 + 成本函数,全部经引擎适配器接入,agent 不维护第二套。
...
|---|---|
| git commit | `1524b0b` |
| 数据指纹 | `74f0b47dbbe28260` |
| 工具数 | 82 |
| 离线断言 | 391 项 |
| 工具 schema | 37471 chars(预算 40500) |
| system prompt | 5674 chars(预算 5700) |
| 最近刷新(UTC) | 2026-08-16T04:38:43Z |
...
- agent 层 24 案例已用 deepseek-v4-flash 在手工集上跑过两场全量(8/24 → 22/24);
  AG-014/020 复跑已过,整场 24 未重跑,不能写成 24/24。生产就绪仍 BLOCKED;
