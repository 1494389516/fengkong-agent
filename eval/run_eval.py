...
def run_online_drift_layer() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            # Authenticated fixture dispatch is tenant-scoped; write its own
            # temporary dataset output instead of global production logs.
            (base / "out").mkdir(parents=True, exist_ok=True)
            log = base / "out" / "agent_runs.jsonl"
            # Preserve the original assertions and restore this fixture's log.
...
def run_decision_plane_layer() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            checks += [
                ("champion 上线无分数:判定不变,仅期望血缘且显式降级",
                 r_ns["expected_model_version"] == "xgb_dp 1"
                 and "model_version" not in r_ns
                 and r_ns["components"]["model"]["status"] == "unavailable"
                 and r_ns["degraded"]
            ...
            (base / "model_scores.json").write_text(
                json.dumps({"u_1001": {"score": 0.3, "model_version": "xgb_dp 1"}}), encoding="utf-8")
            ...
            checks.append(("champion 低分(<review 阈值):不命中 R007",
                           ...
                           and not any(h["rule_id"] == "R007"
                                      for h in r_lo["hits"])))
            (base / "model_scores.json").write_text(
                json.dumps({"u_1001": {"score": 0.99, "model_version": "xgb_dp 1"}}), encoding="utf-8")
