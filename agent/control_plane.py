...
Agent mounts in deployment. This module records desired activation only; a
production deployer/KMS integration must consume these immutable activation events.
"""
import argparse
import copy
import fcntl
    ...
    def propose(self, token, bundle):
        ...
        with self._transaction() as state:
            record = {'id': uuid.uuid4().hex, 'digest': digest(bundle), 'bundle': bundle,
                      ...
                      'baseline_activation': state['active'], 'history': []}
...
def main():
    ...
    controller = ReleaseController(args.store, json.loads(Path(args.credentials).read_text()))
