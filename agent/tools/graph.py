...
def _runtime_limits():
    limits = dict(window_seconds=366 * 86400, max_events=MAX_GRAPH_EVENTS,
                  max_nodes=MAX_GRAPH_NODES, max_resource_degree=MAX_RESOURCE_DEGREE,
                  max_component_nodes=MAX_COMPONENT_NODES, max_component_hops=MAX_COMPONENT_HOPS,
                  max_components=MAX_COMPONENTS, max_expanded_nodes=MAX_EXPANDED_NODES)
    from ..runtime_bundle import current_bundle
    bundle = current_bundle()
    if bundle is not None:
        config = bundle.get("graph")
        if not isinstance(config, dict) or set(config) - set(limits):
            raise ValueError("invalid runtime graph configuration")
...
def _build_graph(as_of_ts=None, window_seconds=DEFAULT_WINDOW_SECONDS) -> nx.Graph:
    ...
    window_seconds = min(window_seconds, limits["window_seconds"])
    g = nx.Graph()
    g.graph.update(limits=limits, as_of_ts=anchor, window_seconds=window_seconds, truncated=False,
                   invalid_events=0, interpretation="association_only")
    for index, e in enumerate(load_events(limit=limits["max_events"] + 1, as_of_ts=anchor,
        ...
        if not _finite_number(ts) or not isinstance(e.get("uid"), str) or not e["uid"]:
            ...
            continue
        recorded_at = e.get("recorded_at", ts)
        if not _finite_number(recorded_at):
            g.graph["invalid_events"] += 1
            continue
        if not anchor - window_seconds <= ts < anchor or recorded_at > anchor:
            continue
        uid = ("uid", e["uid"])
        trust = e.get("identity_trust", "legacy_unverified")
        generation = e.get("entity_generation")
        resources = [(k, e[k]) for k in ("device_id", "ip")
                     if isinstance(e.get(k), str) and e[k].strip()]
        # A reinstall is a new observation resource, never an implicit identity merge.
        if isinstance(generation, str) and generation:
            resources = [(k, "generation:" + json.dumps([v, generation], separators=(",", ":"))
                          if k == "device_id" else v) for k, v in resources]
        ...
        for resource in resources:
            g.add_node(resource, kind=resource[0], resource_id=e.get(resource[0]),
                       entity_generation=generation if resource[0] == "device_id" else None)
            previous = g.get_edge_data(uid, resource, {})
            trusted_device = resource[0] == "device_id" and (
                trust == "legacy_unverified" or
                (trust == "server_bound" and isinstance(generation, str) and bool(generation)))
            g.add_edge(uid, resource, strong=trusted_device and previous.get("strong", True),
                       ...
                       identity_trust=trust, entity_generation=generation,
                       recorded_at=max(recorded_at, previous.get("recorded_at", recorded_at)),
                       resource_id=e.get(resource[0]),
                       weak_reason=("shared_public_exit" if resource[0] == "ip" else
                                    None if trusted_device else "unverified_identity"))
    for node in list(g):
        if node[0] == "device_id" and g.degree(node) > limits["max_resource_degree"]:
...
def _bounded_component(sg, node):
    limits = sg.graph.get("limits", _runtime_limits())
    found = {node}
    queue = deque([(node, 0)])
    truncated = False
    while queue:
        ...
        for neighbor in sorted(sg[current]):
            if neighbor in found:
                continue
            if depth >= limits["max_component_hops"] or len(found) >= limits["max_component_nodes"]:
                truncated = True
                continue
            found.add(neighbor)
...
def _active_blacklist(g):
    ...
    hits, evidence = {}, []
    for kind, value in g:
        canonical = g.nodes[(kind, value)].get("resource_id", value)
        for record in active_records(kind, canonical, g.graph["as_of_ts"], lists=("black", "gray")):
            key = (kind, value)
            ...
            evidence.append({"dimension": kind, "value": value, "list": record["list"],
                             ...
                             "entity_generation": g.nodes[(kind, value)].get("entity_generation"),
...
def _contextual_info(g, strong, expanded, blacklisted, labels, evidence, truncated=False):
    ...
    info["limitations"] = ["Connectivity is association only, not a malicious label.",
                           ...
                           "Legacy fixture identifiers are unverified offline associations; server-bound generations are separate resources.",
    ...
    for nd in sorted(strong_nodes):
        if nd[0] == "uid":
            for resource in sorted(g[nd]):
                if resource in ext:
                    continue
                if len(ext) >= g.graph["limits"]["max_expanded_nodes"]:
                    g.graph["truncated"] = True
                    return ext
                ext.add(resource)
    return ext
...
def graph_relations(uid: Optional[str] = None, device_id: Optional[str] = None,
    ...
    if uid is not None or device_id is not None:
        ...
        node = (kind, val)
        if kind == "device_id" and node not in g:
            candidates = [n for n, d in g.nodes(data=True)
                          if n[0] == kind and d.get("resource_id") == val]
            if len(candidates) > 1:
                return {"error": "ambiguous_device_generation", "device_id": val,
                        "generations": [g.nodes[n].get("entity_generation") for n in candidates],
                        "next_action": "select_generation",
                        "resource_keys": [n[1] for n in candidates]}
            if candidates:
                node = candidates[0]
        if node not in g:
            return {kind: val, "found": False, "next_action": "stop",
                    "as_of_ts": g.graph["as_of_ts"], "window_seconds": g.graph["window_seconds"],
        ...
        for node in sorted(sg):
            ...
            if len(comps) >= g.graph["limits"]["max_components"]:
                g.graph["truncated"] = True
                break
