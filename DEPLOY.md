...
- **红线一点五(服务身份)**:`/brief` 与 `/decide` 必须配置
  ...
  签名，不接受客户端自报的 `X-Operator`。在线 `ts` 只允许有限时钟偏差，
  时序特征以服务端接收时间计算；历史回放不得复用 `/decide`。
- **红线二(权限最小化)**:agent 服务用只读库账号;写操作只能进 pending,
  审批走人;审计进正式审计库。`execute` 状态写必须由用户明确点名具体动作，
  多进程写入和审批共用状态锁；本地审批恢复日志仍应替换为数据库事务。
- **红线三(注入防线)**:举报文本、申诉 claim 等用户可控字段经 dispatch 单点包
