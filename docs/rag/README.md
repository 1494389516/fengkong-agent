...
- Collector 已对固定源码对应的 SDK 7.3.0 短字段实现有限检测投影，支持顶层/递归配置映射；新任务冻结解码证据，未知或非法载荷显式标记缺口。模型仅接收审核词表和类型化状态，原始检测自由文本留在本地。默认 armor 上报仍未贯通，详见 [SDK 载荷接入与验收边界](SDK_PAYLOAD.md)。
