...
1. **效果评估**:agent 的每个能力都有离线断言钉住(当前 380 项,全离线零 token),
...
| git commit | `487d4fa` |
| 工具数 | 82 |
| 工具 schema | 37297 chars |
| system prompt | 5677 chars |
...
flowchart TB
    ...
    T2 --> POL --> DS
    EVAL[eval/ 380 项离线断言 + agent 层四维案例 + 成本预算] -. 回归门禁 .-> 工具层 & AGENT
```
...
**离线(380 项,零 token,CI 门禁)**:规则回归(含防泄漏/误伤守卫)、指标基线、
...
的数学事实,防重构悄悄改坏)、**标注数据质量**(枚举硬校验 + 规则-标签冲突清单,SOP 见 data/labeling_sop.md)、**防御纵深**(规则盲区攻击须被监控层抓住)、
...
agent 运行日志新增延迟计量(llm/tool/total,ms)与按工具聚合;`agent_metrics`
输出 p50/p95/p99/avg/max,并按预算阈值检出 **budget violation**(token 与
